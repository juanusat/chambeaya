...
(function () {
    ...
    async function init() {
        ...
        try {
            ...
            // Agrupar los resultados por ID de publicación para manejar múltiples imágenes
            const publicationsData = {}; // Usar un objeto como mapa
            data.forEach(item => {
                // Si la publicación no está registrada, se inicializa.
                if (!publicationsData[item.id]) {
                    publicationsData[item.id] = {
                        ...item, // Se copian todas las propiedades de la fila actual
                        archivos: [] // Y se crea un array vacío para las imágenes
                    };
                }
                // Si la fila actual tiene un nombre de archivo, se agrega al array correspondiente.
                if (item.archivo) {
                    publicationsData[item.id].archivos.push(item.archivo);
                }
            });
