...
function renderContracts(contractsToRender) {
    ...
    contractsToRender.forEach(contrato => {
        ...
        if (esPrestador) {
            profileImageUrlToDisplay = getProfileImageUrl(contrato.imagenC);
        ...
        // Ahora aparece si el estado es "completado" O "finalizado" Y el usuario actual es el cliente
        if ((contrato.estado?.toLowerCase() === 'completado' || contrato.estado?.toLowerCase() === 'finalizado') && esCliente) {
            ...
            // Si no se cumplen las condiciones, simplemente no se añade el bloque de comentarios.
        }
...
function attachEventListeners() {
    if (contratosContainer) {
        ...
        // Nuevo event listener para el botón "Completado"
...
function handleContractAction(contratoId, estado) {
    ...
    .catch(error => {
        ...
        console.error(`Hubo un error al ${estado} el contrato: ${error.message}`);
    });
}
...
.catch(error => {
    ...
    if (contratosContainer) {
        if (!error.message.includes('Redireccionado')) {
